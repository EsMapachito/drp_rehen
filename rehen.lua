...
--RegisterCommand("miedodrp", function(source, args, rawCommand) -- slash COMMAND
RegisterCommand("rehen", function(source, args)

            if miedodrp ~= 0 then
                SetEntityAsMissionEntity(miedodrp)
                DeleteObject(nativemiedodrpprop)
                miedodrp = 0
                end
                local playerPed = PlayerPedId()
                Citizen.Wait(0)
                ClearPedTasksImmediately(PlayerPedId())
                TaskStartScenamiedodrpInPlace(playerPed, GetHashKey('WORLD_HUMAN_FLEE'), -1, true, false, false, false)
